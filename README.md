<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=150&color=gradient&customColorList=6&section=header&text=Estefany%20Zambrano&fontSize=40&fontColor=ffffff"/>

<div align="center">
  <img src="https://github.com/estefanyZ13.png" width="130" style="border-radius:50%"/>
</div>

<h3 align="center">Estudiante de Desarrollo de Software | UTP - Universidad Tecnológica de Panamá 🇵🇦</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1000&color=8A2BE2&center=true&vCenter=true&width=500&lines=Android+%2F+Kotlin+Developer;.NET+%2F+C%23+Enthusiast;Python+%2B+SQL;Aprendiendo+PHP+%2B+MySQL;Siempre+debuggeando+algo+%F0%9F%90%9B" alt="Typing SVG" />
</p>

---

### 👤 Sobre mí

Estudiante de software enfocada en aprender haciendo. Me gusta entender cómo se conectan las piezas — desde una app de Android hasta una API con su cliente MVC.

### 🌱 Actualmente aprendiendo

Profundizando en Python + SQL, y consolidando lo que ya sé de Android/Kotlin y .NET.

---

### 🛠️ Stack Tecnológico

<p align="center">
  <img src="https://skillicons.dev/icons?i=kotlin,androidstudio,python,php,mysql,git,github" />
</p>

---

### 📌 Proyectos Destacados

- 📱 **MyTimer** — App Android (Kotlin) para calcular horas trabajadas y ganancias según la ley laboral panameña.
- 🌐 **ApiProductos + ProyectoMVC** — Arquitectura ASP.NET Core Web API + MVC, comunicación vía HttpClient y Entity Framework Core.
- 🎬 **Movie/Series Platform** — Plataforma de recomendaciones en PHP + MySQL (XAMPP).
- 👥 **rh_sistema** — Sistema de gestión de RRHH en PHP con roles y autenticación.

---

### 📊 Estadísticas de GitHub

<p align="center">
  <img src="https://github-stats-extended.vercel.app/api?username=estefanyZ13&show_icons=true&theme=tokyonight&hide_border=true&title_color=8A2BE2&icon_color=2F81F7" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats-eight.vercel.app/?user=estefanyZ13&theme=tokyonight&hide_border=true&ring=8A2BE2&fire=2F81F7" />
</p>

---


</div>

<details align="center">
<b>📬 Más información / Contacto</b>
<br>

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-dd4b40?style=for-the-badge&logo=gmail&logoColor=fff)](mailto:estefaniesmith73@gmail.com)
[![Outlook](https://img.shields.io/badge/Outlook-0078D4?style=for-the-badge&logo=microsoftoutlook&logoColor=fff)](mailto:estefany.zambrano@utp.ac.pa)
![Discord](https://img.shields.io/badge/Discord-FannyZ-5865F2?style=for-the-badge&logo=discord&logoColor=fff)

</div>
</details>
